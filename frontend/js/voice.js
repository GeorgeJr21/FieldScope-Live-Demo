...
(() => {
  ...
  function initVoiceCapture() {
    ...
    // La Live Demo no presenta voz como funcional si QVAC está deshabilitado.
    Api.health().then((health) => {
      if (!health.qvac_enabled) {
        button.disabled = true;
        button.setAttribute("aria-label", "Dictado disponible solo en la demo local con QVAC");
        button.title = "Dictado disponible en la versión local con QVAC";
        const status = document.getElementById("voiceStatus");
        if (status) status.textContent = "Voz disponible únicamente en la versión local con QVAC.";
      }
    }).catch(() => {});
  }
  ...
  async function startRecording() {
    ...
    try {
      ...
      notify(
        ...
        denied
          ? "Permite el micrófono para este sitio y vuelve a intentarlo."
