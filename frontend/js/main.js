...
(function () {
  ...
  async function checkSystemStatus() {
    ...
    const backendEndpoint = document.getElementById("backendEndpointLabel");

    if (backendEndpoint && !["127.0.0.1", "localhost"].includes(window.location.hostname)) {
      backendEndpoint.textContent = "Live Demo · HTTPS";
    }
    ...
    try {
      ...
      } else if (health.extraction_mode === "deterministic_dev_fallback") {
        if (aiLabel) {
          aiLabel.textContent = "Extractor determinístico · Demo web";
        }

        if (globalStatus) {
          globalStatus.textContent = "Live Demo · Datos sintéticos";
        }
      ...
      if (aiLabel) aiLabel.textContent = "Revisa la conexión con el backend";
