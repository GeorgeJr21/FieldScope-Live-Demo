...
from config import (MAX_FOLLOW_UP_ROUNDS, MAX_CAPTURE_ROUNDS, MAX_TEXT_LENGTH,
                    ...
                    REFRESH_OPPORTUNITY_YEARS, CORS_ORIGINS, PUBLIC_DEMO, AUTO_SEED_DEMO)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # En PUBLIC_DEMO se usa explícitamente el extractor determinístico.
        print(f"[QVAC] No disponible al iniciar: {exc}")

    if PUBLIC_DEMO and AUTO_SEED_DEMO:
        db = SessionLocal()
        try:
            result = seed_database(db)
            print(f"[DEMO] {result.get('message', 'Dataset revisado.')}")
        finally:
            db.close()
...
async def local_request_guard(request, call_next):
    # La versión local continúa bloqueada a localhost. PUBLIC_DEMO permite que
    # Render entregue la app por HTTPS; el frontend usa la misma origin.
    if not PUBLIC_DEMO:
...
# -------------------- PUBLIC LIVE DEMO STATIC FRONTEND --------------------
# Se monta al final para que /api/* conserve prioridad sobre los archivos web.
if PUBLIC_DEMO:
    from fastapi.staticfiles import StaticFiles

    _frontend_dir = Path(__file__).resolve().parent.parent / "frontend"
    app.mount("/css", StaticFiles(directory=_frontend_dir / "css"), name="demo-css")
    app.mount("/js", StaticFiles(directory=_frontend_dir / "js"), name="demo-js")
