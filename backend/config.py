...
PUBLIC_DEMO = os.getenv("PUBLIC_DEMO", "false").strip().lower() in {"1", "true", "yes", "on"}
AUTO_SEED_DEMO = os.getenv("AUTO_SEED_DEMO", "false").strip().lower() in {"1", "true", "yes", "on"}

# El frontend público se sirve desde el mismo proceso FastAPI, por lo que no
# necesita CORS. En desarrollo local conservamos los dos orígenes auditados.
CORS_ORIGINS = (
    [] if PUBLIC_DEMO else
